...
# 2.2 :Fonction de Préparation,Modelisation, Detection et Visualisation d'anomalie
...
def preparation_reshaping_modelisation_data(data,lags,model,n,THRESHOLD_Inf,THRESHOLD_Sup):
    ...
    erreurs=y_or-y_pred
    print("Erreur de prevision :%.2f"%(mean_squared_error(y_or,y_pred)*100)+'%')
    ...
    elapsed=time()-start
    print("Durée d'exécution:%.2f"%(elapsed))
...
# PARTIE N°3: Pipeline
lags,n=5,1
THRESHOLD_Inf,THRESHOLD_Sup =-0.2 ,0.2
THRESHOLD=0.1
model = load_model('modelRnnLSTMProjetSecurite.h5') # importer le modèle 
modelauto = load_model('modelAutoEncoderProjet.h5') 
...
if args["methode"]=="autoencoder":
    while(1):
        ...
        while(1):
            ...
            sleep(10)
        sleep(180) #pour la maintenance tous les 7 jours soit 24*60*60*7
else:
     while(1):
